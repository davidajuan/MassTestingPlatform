...
There are a few configurable options you can set depending on the environment you are trying to create.
Environment variables are listed in the base file: `.sample.env`. Any env var used within the application is set in this file.
In order to customize the env vars, you will need to copy `.sample.env` and create a `.env` file. Within this new file, you will be able to configure any
variable.
...
| ------------- |:---------------:| -------- |
| `DEBUG_MODE`  | `true`, `false` | Toggles ability to show applications errors to the frontend UI |
| `APP_ENV`     | `prod`, `test`  | Determines what environment to run in. Some 3rd party integrations and processes only run in `prod`. File logging only happens in `test`, but stdout logging is always turned on regardless of environment |
| `GOOGLE_MAPS_API_KEY`  |  | Setting Google Maps API key will enable autocomplete for address, it will be disabled otherwise |
